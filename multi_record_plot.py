...
def main():
    ...
    ap.add_argument("--seconds", type=float, default=3.0, help="Duración de cada toma [s]")
    ap.add_argument("--takes", type=int, default=6, help="Número de tomas")
    ...
    outdir = args.outdir or f"multi_takes_{stamp}"
    ...
    for k in range(args.takes):
        try:
            input(f"\n>> Toma {k+1}/{args.takes}: Presiona Enter para grabar…")
        except EOFError:
            pass
        ...
        wav_path = os.path.join(outdir, f"take_{k+1}.wav")
        ...
        ax1.set_title(f"Toma {k+1} - Tiempo")
        fig1.tight_layout()
        fig1.savefig(os.path.join(outdir, f"take_{k+1}_time.png"), dpi=150)
        plt.close(fig1)

        # Gráfica de frecuencia
        freqs, mag = fft_mag(x, sr); sel = freqs <= args.fmax
        ...
        ax2.set_title(f"Toma {k+1} - FFT")
        fig2.tight_layout()
        fig2.savefig(os.path.join(outdir, f"take_{k+1}_fft.png"), dpi=150)
        plt.close(fig2)
    ...
    if takes:
        ...
        ax.grid(True); ax.set_xlim(0, args.fmax)
        ax.set_xlabel("Frecuencia (Hz)"); ax.set_ylabel("Amplitud")
        ax.set_title("Comparación de FFTs entre tomas")
        ax.legend(fontsize=8)
        fig.tight_layout()
        fig.savefig(os.path.join(outdir, "ffts_comparadas.png"), dpi=150)
        plt.close(fig)
        print(f"[plot] Guardada {os.path.join(outdir, 'ffts_comparadas.png')}")
